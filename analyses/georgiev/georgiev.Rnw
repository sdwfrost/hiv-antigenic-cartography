...
need to work on setting censoring to 5

also need to extract info about sera and viruses
...
<<eval=FALSE>>=
install.packages("rjags", configure.args="--with-jags-include=/usr/local/include/JAGS --with-jags-lib=/usr/local/lib --with-jags-modules=/usr/local/lib/JAGS/modules-3")
@
...
<<readhivdata>>=
hivdata <- read.table("georgiev.neut.txt",header=T,row.names=NULL)
hiv.df <- hivdata[,2:23]
...
pdf("georgievtable.pdf",width=18,height=12)
par(mar=c(5,7,5,5))
...
hivdat$lod[,2] <- hivdat.max-log2(5)+eps
...
<<mapdatabmds,fig=TRUE>>=
hiv.bmds.xmat <- mapdata.mds(hivdat.orig,hiv.bmds.samples.mcmclist,seq(1,1000,by=1)) # NB:try this
...
plot(hiv.bmds.xmat.wgpa.rot,type="n",xlab="D1",ylab="D2",main="BMDS (normal prior)",xlim=c(-3,5),ylim=c(-0.5,0.5))
#text(x=hiv.bmds.xmat.wgpa.rot[1:23,1],y=hiv.bmds.xmat.wgpa.rot[1:36,2],labels=hiv.names[1:36],cex=0.75,col="blue")
points(x=hiv.bmds.xmat.wgpa.rot[1:21,1],y=hiv.bmds.xmat.wgpa.rot[1:21,2],pch=1,cex=0.75,col="blue")
text(x=hiv.bmds.xmat.wgpa.rot[22:43,1],y=hiv.bmds.xmat.wgpa.rot[22:43,2],labels=sera.names,cex=0.75,col="red")
@ 
...
<<>>=
i <- 10
plot(hiv.bmds.xmat.wgpa$rotated[1:43,1:2,i]%*%hiv.bmds.prcomp$rotation)
@
...
for(i in 1:1000){
hiv.bmds.xmat.wgpa.rotated2[1:43,1:2,i] <- hiv.bmds.xmat.wgpa$rotated[1:43,1:2,i]%*%hiv.bmds.prcomp$rotation
}
@
...
names(d) <- c("V1","V2")
#d$V1 <- -d$V1
d$V3 <- c(rep("Virus",21),rep("Sera",22)) # check
p <- ggplot(d,aes(x=V1,y=V2))+coord_fixed(ratio=1)
p <- p+scale_x_continuous(breaks=seq(-3,5),limits=c(-3,5))+scale_y_continuous(breaks=seq(--0.5,0.5),limits=c(-0.5,0.5))
...
for(i in 1:43){
  d1 <- as.data.frame(t(hiv.bmds.xmat.wgpa.rotated2[i,1:2,1:1000]))
...
pdf("georgievmap.pdf",width=8,height=2)
p+facet_grid(V3 ~ .)
dev.off()
@
...
<<>>=
save.image(file="georgiev.RData")
@ 
