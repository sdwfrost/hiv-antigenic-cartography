...
<<readhivdata>>=
hiv <- read.table("vangils2.csv",header=T,row.names=1,sep="\t")
hiv.names <- row.names(hiv)
sera.names <- names(hiv)
...
hmmat <- log2(hivfigmat[1:23,])
hm <- heatmap(hmmat)
hivfigmat2 <- hivfigmat[c(hm$rowInd,seq(24,28)),hm$colInd]
hiv.names2 <- hiv.names[c(hm$rowInd,seq(24,28))]
hiv.subtypes2 <- c(hiv.subtypes[c(hm$rowInd)],rep("Reference B",5))
sera.names2 <- sera.names[hm$colInd]
vals <- hiv[c(hm$rowInd,seq(24,28)),hm$colInd]
...
<<prepdata>>=
hivmat <- readneut(hiv[1:23,],logtransform=TRUE) # exclude reference B strains
hivdat <- prepdata(hivmat)
hivdat.orig <- hivdat # Handy as JAGS overwrites the data passed to it
@ 


I now set up the censoring and the autologous (reference) virus/sera combinations.
...
<<>>=
plot(hiv.bmds.samples.mcmclist$tau.d)
plot(hiv.bmds.samples.mcmclist$tau.x0)
plot(hiv.bmds.samples.mcmclist$delta[1])
@




I fit BPCA; need to check this. This doesn't work here....

<<hivbpca>>=
hivdat.bpca <- prepdata(hivmat)
hivdat.bpca$lod[,2] <- hivdat.max+eps
hivdat.bpca$lod[,1] <- rep(log2(20)-eps,hivdat.bpca$M)
hivdat.bpca$lod[(!is.na(hivdat.bpca$d))&(hivdat.bpca$d<log2(40)),1] <- (hivdat.bpca$d-eps)[!is.na(hivdat.bpca$d)&(hivdat.bpca$d<log2(20))]
hiv.bpca <- jags.model('bpca.bug',data=hivdat.bpca,inits=list(.RNG.name="base::Mersenne-Twister",.RNG.seed=1),n.chains=1,n.adapt=1000)
...
text(hiv.nocen.bmds.xmat.wgpa$mshape,labels=hiv.names[1:23])
@ 
...
mycols <- rainbow(5)
plot(hiv.bmds.xmat.wgpa.rot,type="n",xlab="D1",ylab="D2",main="BMDS (normal prior)",xlim=c(-6,6),ylim=c(-0.5,0.5))
text(x=hiv.bmds.xmat.wgpa.rot[1:23,1],y=hiv.bmds.xmat.wgpa.rot[1:23,2],labels=hiv.subtypes[1:23],cex=0.75,col="blue")
...
Outlying sera? B's clustered near x=0?

Now rotate all the MCMC runs. Look for convergence issues.

<<>>=
i <- 10
plot(hiv.bmds.xmat.wgpa$rotated[1:38,1:2,i]%*%hiv.bmds.prcomp$rotation)
@
...
for(i in 1:100){
hiv.bmds.xmat.wgpa.rotated2[1:38,1:2,i] <- hiv.bmds.xmat.wgpa$rotated[1:38,1:2,i]%*%hiv.bmds.prcomp$rotation
}
@
...
names(d) <- c("V1","V2")
d$V3 <- c(rep("Virus",23),rep("Sera",15)) # check
