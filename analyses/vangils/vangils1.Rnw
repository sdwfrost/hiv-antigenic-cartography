...
<<readhivdata>>=
hiv <- read.csv("vangils.csv",header=T,row.names=1)
...
I now prepare the data for analysis, using only the 36 patients, as well as with JRCSF.

<<prepdata>>=
hivmat <- readneut(hiv[1:23,],logtransform=TRUE)
...
The above doesn't show any reak clustering by subtype...perhaps if I look at the 'subtype B' responders.
