...
<<readhivdata>>=
hiv1 <- read.csv("vangils1.csv",header=T,row.names=1)
hiv2 <- read.table("vangils2.csv",header=T,row.names=1,sep="\t")
hiv3 <- read.table("vangils3.csv",header=T,row.names=1,sep="\t")
hiv <- cbind(hiv1,hiv2,hiv3)
...
hiv.subtypes2 <- c(hiv.subtypes[c(hm$rowInd)])
...
pdf("vangilstable.pdf",width=20,height=12)
par(mar=c(5,7,5,7))
...
hivdat$lod[,2] <- hivdat.max-log2(40)+eps
...
hivdat.bpca$lod[,1] <- rep(log2(40)-eps,hivdat.bpca$M)
dinit <- rep(log2(40)-2*eps,length(hivdat.bpca$d))
dinit[hivdat.bpca$cen==1] <-  NA # censored measurements
dinit[c(52,56,80,81,84,112,352,924)] <- log2(40) # ND measurements
...
text(x=hiv.bmds.xmat.wgpa.rot[1:28,1],y=hiv.bmds.xmat.wgpa.rot[1:28,2],labels=hiv.subtypes,cex=0.75,col="blue")
...
text(x=hiv.bmds.xmat.wgpa.rot[29:63,1],y=hiv.bmds.xmat.wgpa.rot[29:63,2],labels=names(hiv),cex=0.75,col="red")
@ 
