...
set.seed(1)
rndcol <- sort(sample(seq(1,103),35))
hiv <- hiv[,rndcol]
...
text(x=hiv.bmds.xmat.wgpa.rot[21:(21+35-1),1],y=hiv.bmds.xmat.wgpa.rot[21:(21+35-1),2],labels=dimnames(hiv)[[2]],cex=0.75,col="red")
@ 

Note that the sera and viruses are separate...this time, however, there is more variation in the y axis. Perhaps one could compare the hyperparameters for the different models? Due to lower LOD? Need to plot out different subtypes too.
