...
hivdat$cen[hivdat$d<log2(20)] <- 2
hivdat$d[hivdat$d<log2(20)] <- NA
...
hivdat$lod[,2] <- hivdat.max-log2(20)+eps
...
points(x=hiv.bmds.xmat.wgpa.rot[1:20,1],y=hiv.bmds.xmat.wgpa.rot[1:20,2],pch=1,cex=0.75,col="blue")
text(x=hiv.bmds.xmat.wgpa.rot[21:123,1],y=hiv.bmds.xmat.wgpa.rot[21:123,2],labels=dimnames(hiv)[[2]],cex=0.75,col="red")
@ 

Note that the sera and viruses are separate...this time, however, there is more variation in the y axis. This is NOT due to the lower LOD, as we have got rid of this.

Plot out the second dimension.

<<fig=TRUE>>=
hist(hiv.bmds.xmat.wgpa.rot[21:123,2])
@ 

It's quite a heavy tailed distribution, so getting lots of sera makes sense. I will try to thin the number of sera down to (say) 20.

Perhaps one could compare the hyperparameters for the different models? Need to plot out different subtypes too.

Now save data.

<<>>=
save.image(file="doriarose.RData")
@ 
